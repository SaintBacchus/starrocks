...
#include "converter/starrocks_arrow_converter.h"
...
public:
    StarRocksFormatReaderImpl(int64_t tablet_id, int64_t version, std::shared_ptr<arrow::Schema> required_schema,
                              std::shared_ptr<arrow::Schema> output_schema, std::string tablet_root_path,
        ...
        LOG(INFO) << " Open tablet schema " << _output_schema->ToString();
    }

    arrow::Status open() override {
        LOG(INFO) << " Open tablet reader " << _tablet_id << " version: " << _version
        ...
        FORMAT_ASSIGN_OR_RAISE_ARROW_STATUS(auto metadata,
                                            _lake_tablet_manager->get_tablet_metadata(fs, metadata_location, true));
        ...
        LOG(INFO) << " table schema is " << _tablet_schema->debug_string();
        // get scan column index from tablet schema.
        std::vector<uint32_t> required_column_indexs;
        ARROW_RETURN_NOT_OK(schema_to_column_index(_required_schema, required_column_indexs, using_column_uid));
        ...
        ARROW_RETURN_NOT_OK(schema_to_column_index(_output_schema, output_column_indexs, using_column_uid));
        ...
        if (std::equal(_scan_column_indexs.begin(), _scan_column_indexs.end(), output_column_indexs.begin(),
            ...
            _need_project = true;
            _output_tablet_schema = std::make_shared<starrocks::Schema>(
                    ChunkHelper::convert_schema(_tablet_schema, output_column_indexs));
            LOG(INFO) << "OutputSchema and ScanSchema are not same. OutputSchema size is "
                      << output_column_indexs.size() << " and ScanSchema size is " << _scan_column_indexs.size() << ".";
            ARROW_RETURN_NOT_OK(build_output_index_map(_output_tablet_schema, _scan_schema));
        }

        LOG(INFO) << "schema  addr " << *_output_tablet_schema;
    ...
    arrow::Status get_next(struct ArrowArray* c_arrow_array) override {
        auto chunk = ChunkHelper::new_chunk(*_output_tablet_schema, _chunk_size);
        Status status = do_get_next(chunk);

        std::shared_ptr<arrow::RecordBatch> rbatch;
        if (status.ok()) {
            ARROW_ASSIGN_OR_RAISE(rbatch, _arrow_converter->convert(chunk.get()))
        } else if (status.is_end_of_file()) {
            LOG(INFO) << "no more data in tablet " << _tablet_id;
            ARROW_ASSIGN_OR_RAISE(rbatch, arrow::RecordBatch::MakeEmpty(_output_schema));
        } else {
        ...
        if (rbatch) {
            return arrow::ExportRecordBatch(*rbatch, c_arrow_array);
        } else {
            return arrow::Status::OK();
        }
    }
...
private:
    arrow::Status build_output_index_map(const std::shared_ptr<starrocks::Schema>& output,
    ...
    arrow::Result<int32_t> get_column_id(const std::shared_ptr<arrow::Field>& field) {
        ...
        if (!field->HasMetadata() || !metadata->Contains("starrokcs.columnId")) {
            return arrow::Status::Invalid("invalid field unique id.");
        }

        try {
            ARROW_ASSIGN_OR_RAISE(string str_unique_id, metadata->Get("starrokcs.columnId"));
    ...
    Status init_reader_params(TabletReaderParams& params) {
        ...
        RETURN_IF_ERROR(
                _conjuncts_manager.parse_conjuncts(true, config::max_scan_key_num, enable_column_expr_predicate));
...
private:
    ...
    std::shared_ptr<starrocks::Schema> _output_tablet_schema;
...
arrow::Result<StarRocksFormatReader*> StarRocksFormatReader::create(
        int64_t tablet_id, int64_t version, struct ArrowSchema* required_arrow_schema,
        struct ArrowSchema* output_arrow_schema, std::string tablet_root_path,
        std::unordered_map<std::string, std::string> options) {
    ...
    return create(tablet_id, version, required_schema, output_schema, std::move(tablet_root_path), std::move(options));
}

arrow::Result<StarRocksFormatReader*> StarRocksFormatReader::create(
        int64_t tablet_id, int64_t version, std::shared_ptr<arrow::Schema> required_schema,
        std::shared_ptr<arrow::Schema> output_schema, std::string tablet_root_path,
        std::unordered_map<std::string, std::string> options) {
    StarRocksFormatReaderImpl* format_Reader = new StarRocksFormatReaderImpl(
            tablet_id, version, required_schema, output_schema, std::move(tablet_root_path), std::move(options));
