...
#include "converter/binary_converter.h"
#include "converter/column_converter.h"
#include "converter/nested_converter.h"
#include "converter/primitive_converter.h"
...
arrow::Result<std::shared_ptr<ColumnConverter>> ColumnConverter::create(
        const std::shared_ptr<arrow::DataType> arrow_type, const std::shared_ptr<starrocks::Field> sr_field,
...
#define CONVERTER_CASE(ARROW_TYPE_ID, SR_TYPE_ID, CONVERTER_CLASS_TYPE) \
    ...
    default: {
        return arrow::Status::NotImplemented("Do not support conversion ", arrow_type->ToString(), " <--> ",
                                             sr_field->to_string());
    }
...
#undef CONVERTER_CASE_IMPL
    ...
    //             return arrow::Status::Invalid("StarRocks sub fields is empty while arrow's sub fields is not empty.",
    //                                           arrow_type->num_fields());
    //         }
    //         if (arrow_type->num_fields() != sr_field->sub_fields().size()) {
    //             return arrow::Status::Invalid("number fields not match with arrow:", arrow_type->num_fields(),
    //                                           ", sr:", sr_field->sub_fields().size());
    //         }

    //         LOG(INFO) << " converter children";
    //         LOG(INFO) << " converter name is " << sr_field->name();
    //         // create children converters for map, array, struct etc.
    //         for (size_t idx = 0; idx < arrow_type->num_fields(); idx++) {
    //             std::shared_ptr<starrocks::Field> sr_child_field =
    //                     std::make_shared<starrocks::Field>(sr_field->sub_field(idx));

    //             if (arrow_type->field(idx)->name() != sr_field->sub_field(idx).name()) {
    //                 return arrow::Status::Invalid(
    //                         "child field's name not match with arrow:", arrow_type->field(idx)->name(),
    ...
    //             LOG(INFO) << " child type is " << arrow_type->field(idx)->ToString() << " sr type "
    //                       << sr_field->sub_field(idx);
    //             result->addChild(converter);
    //         }
    //     }
    // }
    if (arrow_type->num_fields() > 0) {
        LOG(INFO) << " child type arrow is " << arrow_type->ToString() << " sr type " << *sr_field;
        if (sr_field->has_sub_fields()) {
            for (auto field : sr_field->sub_fields()) {
                LOG(INFO) << " sr children type is " << field << " length " << field.length();
            }
        }
...
arrow::Result<ColumnConverterVector> ColumnConverter::create_children_converter(
    ...
    if (arrow_type->id() == arrow::Type::LIST) {
         // map field's sub fields should be 1:element.
        if (!sr_field->has_sub_fields() || sr_field->sub_fields().size() != 1) {
            return arrow::Status::Invalid("Invalid sub filelds size for array field ", sr_field->name());
        }
        ...
        auto sr_offset_field = std::make_shared<starrocks::Field>(
                0, "offset", starrocks::get_type_info(starrocks::LogicalType::TYPE_INT), false);
        std::shared_ptr<starrocks::Field> sr_element_field = std::make_shared<starrocks::Field>(sr_field->sub_field(0));
        ...
        result.emplace_back(converter);
    } else if (arrow_type->id() == arrow::Type::MAP) {
        // map field's sub fields should be 2:key and value.
        if (!sr_field->has_sub_fields() || sr_field->sub_fields().size() != 2) {
            return arrow::Status::Invalid("Invalid sub filelds size for array field ", sr_field->name());
        }
        // arrow children datatype
        const auto arrow_offset_datatype = arrow::int32();
        ...
        auto sr_offset_field = std::make_shared<starrocks::Field>(
                0, "offset", starrocks::get_type_info(starrocks::LogicalType::TYPE_INT), false);
        auto sr_key_field = std::make_shared<starrocks::Field>(
                0, "key", starrocks::get_key_type_info(sr_field->type().get()), false);
        auto sr_value_field = std::make_shared<starrocks::Field>(
                0, "value", starrocks::get_value_type_info(sr_field->type().get()), false);
        ARROW_ASSIGN_OR_RAISE(auto converter, ColumnConverter::create(arrow_offset_datatype, sr_offset_field, pool));
        result.emplace_back(converter);
        ...
        if (arrow_type->num_fields() != sr_field->sub_fields().size()) {
            return arrow::Status::Invalid("number fields not match with arrow size :", arrow_type->num_fields(),
                                          ", sr size :", sr_field->sub_fields().size());
        }

        for (size_t idx = 0; idx < arrow_type->num_fields(); ++idx) {
            if (arrow_type->field(idx)->name() != sr_field->sub_field(idx).name()) {
                return arrow::Status::Invalid("child field's name not match with arrow:",
                                              arrow_type->field(idx)->name(), ", sr:", sr_field->sub_field(idx).name());
            }
            std::shared_ptr<starrocks::Field> sr_child_field =
                    std::make_shared<starrocks::Field>(sr_field->sub_field(idx));
...
// convert starocks null data to arrow null bitmap
// arrow use 0 as null indicater and use bit
// while sr use 1 as null indicater and use byte
arrow::Result<std::shared_ptr<arrow::Buffer>> ColumnConverter::convert_nullbitmap(
        const std::vector<uint8_t>& null_bytes) {
    std::shared_ptr<arrow::Buffer> null_bitmap;
    size_t num_rows = null_bytes.size();
    ARROW_ASSIGN_OR_RAISE(null_bitmap, arrow::internal::BytesToBits(null_bytes, const_cast<arrow::MemoryPool*>(_pool)));
...
std::shared_ptr<starrocks::Column> ColumnConverter::get_data_column(const std::shared_ptr<starrocks::Column>& column) {
