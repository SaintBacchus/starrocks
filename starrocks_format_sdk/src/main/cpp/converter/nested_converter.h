...
class NestedConverter : public ColumnConverter {
    ...
    //
public:
    NestedConverter(const std::shared_ptr<arrow::DataType> arrow_type, const std::shared_ptr<starrocks::Field> sr_field,
    ...
    arrow::Result<std::shared_ptr<arrow::Array>> toArrowArray(const std::shared_ptr<starrocks::Column>& column) {
        ...
        if (column->is_nullable()) {
            ...
            ARROW_ASSIGN_OR_RAISE(null_bitmap, convert_nullbitmap(null_bytes));
        }
    ...
    arrow::Status toSrColumn(const std::shared_ptr<arrow::Array> array, std::shared_ptr<starrocks::Column>& out) {
        ...
        if (arrow_children_arrays.size() != sr_sub_columns.size()) {
            return arrow::Status::Invalid(
                    "Can not convert nested array because array children size(", arrow_children_arrays.size(),
                    ") is not same as starrocks complext sub-column size (", sr_sub_columns.size(), ")! ");
        }
        ...
        LOG(INFO) << " nested array row num " << array->length() << "  arrow content:" << array->ToString();
        LOG(INFO) << " nested column row num " << out->size() << "  content:" << out->debug_string();
        LOG(INFO) << " nested column arrow null countrow num " << out->size();
...
private:
    ...
    arrow::Result<arrow::ArrayVector> get_children_arrays(const std::shared_ptr<ArrowArrayClass> array) {
        if constexpr (std::is_same_v<ArrowArrayClass, arrow::ListArray>) {
            ...
            LOG(INFO) << "list child array offset content's " << array->offsets()->ToString();
            return all_arrays;
        } else if constexpr (std::is_same_v<ArrowArrayClass, arrow::MapArray>) {
            LOG(INFO) << "map child array offset content's " << array->offsets()->ToString();
    ...
    template <class SrColumnClass, typename = std::enable_if_t<std::is_same_v<SrColumnClass, starrocks::ArrayColumn> ||
                                                               std::is_same_v<SrColumnClass, starrocks::MapColumn> ||
                                                               std::is_same_v<SrColumnClass, starrocks::StructColumn>>>
    arrow::Result<std::vector<starrocks::ColumnPtr>> get_children_columns(
            const std::shared_ptr<SrColumnClass> data_column) {
        if constexpr (std::is_same_v<SrColumnClass, starrocks::ArrayColumn>) {
            LOG(INFO) << "list child column offset content's " << data_column->offsets_column()->debug_string();
            std::vector<ColumnPtr> all_sub_columns = {data_column->offsets_column(), data_column->elements_column()};
            return all_sub_columns;
        } else if constexpr (std::is_same_v<SrColumnClass, starrocks::MapColumn>) {
            LOG(INFO) << "map child column offset content's " << data_column->offsets_column()->debug_string();
            std::vector<ColumnPtr> all_sub_columns = {data_column->offsets_column(), data_column->keys_column(),
                                                      data_column->values_column()};
            return all_sub_columns;
        } else if constexpr (std::is_same_v<SrColumnClass, starrocks::StructColumn>) {
    ...
    arrow::Result<std::shared_ptr<arrow::Array>> make_nested_array(const arrow::ArrayVector arrays,
        ...
        if constexpr (std::is_same_v<ArrowArrayClass, arrow::ListArray>) {
            ...
            return arrow::MapArray::FromArrays(_arrow_type, arrays[0], arrays[1], arrays[2],
