...
arrow::Result<ColumnConverterVector> ColumnConverter::create_children_converter(
    ...
    if (arrow_type->id() == arrow::Type::LIST && sr_field->type()->type() == starrocks::LogicalType::TYPE_ARRAY) {
        ...
        result.emplace_back(converter);
    } else if (arrow_type->id() == arrow::Type::MAP && sr_field->type()->type() == starrocks::LogicalType::TYPE_MAP) {
        ...
        auto sr_offset_field = std::make_shared<starrocks::Field>(
                0, "offset", starrocks::get_type_info(starrocks::LogicalType::TYPE_INT), false);
        auto sr_key_field = std::make_shared<starrocks::Field>(sr_field->sub_field(0));
        auto sr_value_field = std::make_shared<starrocks::Field>(sr_field->sub_field(1));
        ARROW_ASSIGN_OR_RAISE(auto converter, ColumnConverter::create(arrow_offset_datatype, sr_offset_field, pool));
        result.emplace_back(converter);
        ...
        result.emplace_back(converter);
               ...
               sr_field->type()->type() == starrocks::LogicalType::TYPE_STRUCT) {
