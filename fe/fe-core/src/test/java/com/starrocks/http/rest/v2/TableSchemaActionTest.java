...
public class TableSchemaActionTest extends StarRocksHttpTestCase {
    ...
    public void testMissingParameter() throws Exception {
        {
            Request request = newRequest(DEFAULT_INTERNAL_CATALOG_NAME, "", TABLE_NAME);
            try (Response response = networkClient.newCall(request).execute()) {
                RestBaseResultV2<TableSchemaView> resp = parseResponseBody(response.body().string());
                assertEquals("400", resp.getCode());
                assertTrue(resp.getMessage().contains("Missing parameter"));
            }
        }

        {
            Request request = newRequest(DEFAULT_INTERNAL_CATALOG_NAME, DB_NAME, "");
            try (Response response = networkClient.newCall(request).execute()) {
                RestBaseResultV2<TableSchemaView> resp = parseResponseBody(response.body().string());
                assertEquals("400", resp.getCode());
                assertTrue(resp.getMessage().contains("Missing parameter"));
        ...
        {
            Request request = newRequest(DEFAULT_INTERNAL_CATALOG_NAME, DB_NAME, TABLE_NAME);
            try (Response response = networkClient.newCall(request).execute()) {
                RestBaseResultV2<TableSchemaView> resp = parseResponseBody(response.body().string());
    ...
    public void testNonOlapTable() throws Exception {
        ...
        try (Response response = networkClient.newCall(request).execute()) {
            ...
            assertTrue(resp.getMessage().contains("only support olap table currently"));
        }
    }
    ...
    public void testOlapTable() throws Exception {
        ...
        try (Response response = networkClient.newCall(request).execute()) {
            ...
            assertEquals(1, indexMetas.size());
            {
                ...
                assertEquals(testIndexId, indexMeta.getIndexId().longValue());
            ...
            assertEquals(2, indexes.size());
            {
                ...
                assertEquals(IndexDef.IndexType.BITMAP.name(), idx.getIndexType());
            ...
            assertEquals(testIndexId, tableSchema.getBaseIndexId().longValue());
    ...
    private static OlapTable newOlapTable(Long tableId, String tableName) {
        ...
        Index idx1 = new Index("idx1", Lists.newArrayList("c1"), IndexDef.IndexType.BITMAP, "c_idx1");
        Index idx2 = new Index("idx2", Lists.newArrayList("c2"), IndexDef.IndexType.BITMAP, "c_idx2");
        ...
        TableProperty tableProperty = new TableProperty(new LinkedHashMap<String, String>() {
