...
public:
    ColumnConverter(const std::shared_ptr<arrow::DataType> arrow_type, const std::shared_ptr<starrocks::Field> sr_field,
                    const arrow::MemoryPool* pool)
    ...
    virtual arrow::Result<std::shared_ptr<arrow::Array>> toArrowArray(
            const std::shared_ptr<starrocks::Column>& column) = 0;

    virtual arrow::Status toSrColumn(const std::shared_ptr<arrow::Array> array,
                                     std::shared_ptr<starrocks::Column>& out) = 0;

protected:
    arrow::Result<std::shared_ptr<arrow::Buffer>> convert_nullbitmap(const std::vector<uint8_t>& null_bytes);
    arrow::Status convert_nullbitmap(const std::shared_ptr<arrow::Buffer>, const std::vector<uint8_t>& null_bytes);

    std::shared_ptr<starrocks::Column> get_data_column(const std::shared_ptr<starrocks::Column>& column);

protected:
    const std::shared_ptr<arrow::DataType> _arrow_type;
    // when append string data, check length limit will use _sr_field;
    const std::shared_ptr<starrocks::Field> _sr_field;
