...
arrow::Status BaseArrowConverter::init() {
    ...
    for (int i = 0; i < num_fields; i++) {
        ARROW_ASSIGN_OR_RAISE(auto converter,
                              ColumnConverter::create(_arrow_schema->field(i)->type(), _sr_schema->field(i), _pool));
...
public:
    ...
    arrow::Result<std::shared_ptr<arrow::RecordBatch>> convert(const Chunk* chunk) override {
        ...
        std::shared_ptr<arrow::RecordBatch> rb =
                arrow::RecordBatch::Make(_arrow_schema, chunk->num_rows(), std::move(arrays));
        for (size_t rowindex = 0; rowindex < chunk->num_rows(); rowindex++) {
            LOG(INFO) << " convert chunk to rb: chunk is  " << chunk->debug_row(rowindex);
        }
        LOG(INFO) << " convert chunk to rb  " << rb->ToString();
        return rb;
    }
};
...
public:
    ...
    arrow::Result<std::unique_ptr<starrocks::Chunk>> convert(
        ...
        auto num_rows = rbatch->num_rows();

        auto chunk = starrocks::ChunkHelper::new_chunk(*_sr_schema, num_rows);
        for (size_t idx = 0; idx < num_fields; ++idx) {
            ARROW_RETURN_NOT_OK(_converters[idx]->toSrColumn(rbatch->column(idx), chunk->columns()[idx]));
        }
        for (size_t rowindex = 0; rowindex < chunk->num_rows(); rowindex++) {
            LOG(INFO) << " convert rb to column  " << chunk->debug_row(rowindex);
        }
        return chunk;
    }
};
