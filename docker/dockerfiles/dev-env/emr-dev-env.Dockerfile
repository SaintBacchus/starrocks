...
# value: true | false
# default: false
ARG predownload_thirdparty=true
...
COPY ./docker/ve-emr/settings.xml /root/.m2/
COPY . ./starrocks
...
RUN export https_proxy=http://sys-proxy-rd-relay.byted.org:8118 http_proxy=http://sys-proxy-rd-relay.byted.org:8118 no_proxy="*.byted.org" \
     && mkdir -p $STARROCKS_THIRDPARTY/installed && cd starrocks/thirdparty && \
...
RUN mkdir -p /opt/tiger /var/log/tiger  /opt/log/tiger \
    && groupadd tiger && useradd -g tiger tiger \
    && chown -R tiger /opt/tiger /var/log/tiger  /opt/log/tiger \
    && chgrp -R tiger /opt/tiger /var/log/tiger  /opt/log/tiger \
    && usermod -G root tiger
